...
class postController {
  async getPostBySlug(req, res, next) {
    try {
      ...
      const foundPost = await Post.findOne({ slug }).populate({
        path: 'categoryId',
        select: '_id name slug',
      })

      if (!foundPost)
        return next(createError(404, { success: false, message: 'Not found' }))
      ...
      return res.status(200).json({
        success: true,
        post: foundPost,
        postComments: foundPostComment,
        topicComments: foundTopicComment,
      })
    } catch (error) {
  ...
  async getPostsByCategory(req, res, next) {
    try {
      ...
      let regex = new RegExp(
        '^' + (foundCategory.parentId + ',' + foundCategory._id)
      )
      ...
      let foundPosts = await Post.find({
        categoryId: { $in: foundSubCategory },
      })
        .populate({ path: 'categoryId', select: '_id name slug' })
        .limit(4)
      ...
      return res
        .status(200)
        .json({ success: true, posts: foundPosts, category: foundCategory })
    } catch (error) {
      return next(createError(400, { success: false, message: error.message }))
    }
  }
  async getListCategory(req, res, next) {
    try {
      ...
      let foundListCategory = await Category.aggregate([
        {
          $match: {
            parentId: {
              $in: foundParentCategory.map(
                (e) => (e.parentId = e.parentId + ',' + e._id)
              ),
            },
          },
        },
      ...
      return res
        .status(200)
        .json({ success: true, categories: foundListCategory })
    } catch (error) {
      return next(createError(400, { success: false, message: error.message }))
    }
  }
