...
const loginByGoogle = async (req, res) => {
  ...
  try {
    ...
    if (existingUser) {
      ...
      return res.status(200).json({ success: true, message: `login successfully`, token: newToken, user: existingUser });
    } else {
      ...
      return res.status(200).json({ success: true, message: `login successfully`, token: newToken, user: newUser });
    }
  } catch (error) {
    res.status(400).json({ success: false, msg: error.message })
  }
